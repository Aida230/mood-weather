...
const WeatherResult = ({ data }) => {
  if (!data) return null; // Si no hay datos, no se muestra nada
    ...
    <div className="animate-fade-in mt-10 max-w-4xl mx-auto px-4 pb-6">
      <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-white/20 text-left space-y-4">
        ...
        <div className="text-lg space-y-1 text-slate-100">
          <p><span className="font-semibold text-sky-300">📍 Ciudad:</span> {data.city}</p>
          <p><span className="font-semibold text-yellow-300">🌡️ Temperatura:</span> {data.temperature}</p>
          <p><span className="font-semibold text-pink-300">😊 Estado de ánimo:</span> {data.mood}</p>
          <p><span className="font-semibold text-green-300">🏃 Actividad:</span> {getActivityEmoji(data.activity)} {data.activity}</p>
          <p><span className="font-semibold text-purple-300">💬 Frase:</span> “{data.quote}”</p>
        </div>
